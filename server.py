...
BABYSITTER_BIO   = ("Olá! Sou a Henriqueta, tenho 14 anos e adoro cuidar de crianças. "
                    "Tenho experiência com bebés e crianças dos 0 aos 11 anos, curso de "
...
# ⚠️ SEGURANÇA: Nunca coloca chaves diretamente no código!
# Define estas variáveis no Railway em: Service → Variables
...
def ask_chatbot(question: str, history: list) -> str:
    if not ANTHROPIC_API_KEY:
        return "ESCALAR: API não configurada"

    system = f"""És a assistente virtual da {BABYSITTER_NAME}, babysitter em {LOCATION}.
Respondes SEMPRE em português europeu, de forma simpática, direta e útil.
Tenta responder a qualquer pergunta relacionada com os serviços.

Informações que CONHECES e deves usar nas respostas:
- Nome: {BABYSITTER_NAME}
- Preço: {HOURLY_RATE}
- Localização: {LOCATION}
- Experiência: bebés e crianças dos 0 aos 11 anos
- Formação: curso de culinária
- Serviços: cuidado de bebés, recreação, refeições saudáveis, pernoite, apoio escolar, passeios
- Agendamento: pode ser feito diretamente no site na secção "Agendar"
- WhatsApp disponível para contacto: +{WHATSAPP_NUMBER}

REGRAS IMPORTANTES:
1. Responde PRIMEIRO com a informação útil. Só menciona o WhatsApp como alternativa ADICIONAL no final, nunca como resposta principal.
2. Se a pergunta for sobre preços, di "custa {HOURLY_RATE}".
3. Se for sobre serviços, descreve-os.
4. Se for sobre disponibilidade, sugere usar o formulário de agendamento no site.
5. Só usa "ESCALAR: [motivo]" se a pergunta for COMPLETAMENTE impossível de responder com as informações acima (ex: pedidos muito específicos de datas, situações de emergência, reclamações complexas).
6. NUNCA respondas apenas com "contacta pelo WhatsApp" sem dar primeiro uma resposta útil."""
    ...
    try:
        ...
        return "ESCALAR: erro técnico no servidor"
...
def calc_end_time(start_time: str, hours: int) -> str:
    """Calcula hora de fim dado hora de início e número de horas."""
    try:
...
.btn-agent { background: #fffbeb; color: #92400e; border: 1.5px solid #fcd34d; }
.btn-agent:hover { background: #fef3c7; }
...
.time-preview {
  background: var(--pink-l); border-radius: 8px;
  padding: .6rem 1rem; font-size: .85rem; color: var(--pink-d);
  font-weight: 600; margin-top: .4rem; display: none;
}
...
.chat-messages {
  height: 340px; overflow-y: auto; padding: 1.2rem;
...
.chat-escalate {
  ...
  font-size: .85rem; color: #78350f; align-self: flex-start;
  max-width: 85%; display: flex; flex-direction: column; gap: .5rem;
}
.chat-escalate strong { color: #92400e; }
.chat-escalate a {
  display: inline-flex; align-items: center; gap: .4rem;
  background: #25d366; color: #fff; padding: .45rem .9rem;
...
.login-page {
  ...
  justify-content: center; background: var(--cream); padding: 2rem;
}
.login-card {
  background: var(--white); border-radius: var(--radius);
  padding: 3rem; max-width: 420px; width: 100%;
  box-shadow: 0 8px 40px rgba(244,114,182,.12); text-align: center;
}
...
function updateTimePreview() {
  ...
  preview.textContent   = `⏱ Das ${start} às ${endH}:${endM} (${hours}h)`;
}
document.getElementById('start_time')?.addEventListener('input', updateTimePreview);
document.getElementById('hours_input')?.addEventListener('input', updateTimePreview);
...
function appendEscalation() {
  ...
  div.innerHTML = `
    <strong>🙋 Precisas de ajuda humana?</strong>
    <span>Um funcionário pode responder a perguntas mais específicas.</span>
    <div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-top:.25rem">
      <button onclick="requestAgent()" class="btn btn-sm btn-agent">🧑‍💼 Chamar agente</button>
      <a href="https://wa.me/351965813670" target="_blank">💬 WhatsApp</a>
    </div>`;
  chatMessages?.appendChild(div);
  chatMessages?.scrollTo(0, chatMessages.scrollHeight);
}
...
async function requestAgent() {
  try {
    ...
    const eid  = data.eid;
    appendMsg('✅ Pedido enviado! Um agente responderá em breve aqui mesmo neste chat.', 'bot');
    if (eid) startClientPolling(eid);
  } catch {
    appendMsg('Não foi possível enviar o pedido. Tenta pelo WhatsApp.', 'bot');
  }
}
...
function startClientPolling(eid) {
  activeEid = eid;
  lastMsgCount = 0;

  // Mostrar input para o cliente continuar a escrever
  const inputRow = document.querySelector('.chat-input-row');
  if (inputRow) {
    inputRow.innerHTML = `
      <input id="chat-input" type="text" placeholder="Escreve a tua mensagem...">
      <button id="chat-send">Enviar</button>`;
    document.getElementById('chat-send').addEventListener('click', sendClientMsg);
    document.getElementById('chat-input').addEventListener('keydown', e => {
      if (e.key === 'Enter') sendClientMsg();
    });
  }

  if (clientPollInterval) clearInterval(clientPollInterval);
  clientPollInterval = setInterval(() => pollAdminReplies(eid), 2500);
}

async function sendClientMsg() {
  if (!activeEid) return sendChat();
  ...
  appendMsg(text, 'user');
  try {
    await fetch('/api/chat/send/' + activeEid, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function pollAdminReplies(eid) {
  try {
    const res  = await fetch('/api/chat/messages/' + eid);
    const data = await res.json();
    if (data.messages.length > lastMsgCount) {
      const newMsgs = data.messages.slice(lastMsgCount);
      newMsgs.forEach(m => {
        appendMsg('🧑‍💼 Agente: ' + m.message, 'bot');
      });
      lastMsgCount = data.messages.length;
    }
    if (data.resolved) {
      clearInterval(clientPollInterval);
      appendMsg('✅ Esta conversa foi marcada como resolvida. Obrigada!', 'bot');
    }
  } catch { /* silencioso */ }
}
...
async function sendChat() {
  ...
  try {
    ...
    if (data.escalated) {
      ...
      appendEscalation();
      if (data.eid) startClientPolling(data.eid);
    } else {
...
def layout(title, body, session=None):
    user = session
    nav_user = ""
...
{body}
<footer>
  <p>© 2026 {BABYSITTER_NAME} · Lisboa, Portugal ·
...
def page_home(session=None, booking_msg="", booking_type="ok", review_msg="", review_type="ok"):
    ...
    photo_html = (
        f'<img src="{BABYSITTER_PHOTO}" alt="Foto" style="width:100%;height:100%;border-radius:50%;object-fit:cover">'
    ...
    if session and session.get("name"):
        user_fields = f'<input type="hidden" name="user_id" value="{session.get("user_id", "")}">'
...
<section class="hero" id="inicio">
  ...
  <div class="hero-content">
    ...
    <div class="hero-badges fade-up fade-up-2">
      ...
      <span class="badge">🍼 Bebés dos 0–11 anos</span>
  ...
  <div style="max-width:1000px;margin:0 auto">
    ...
    <div class="services-grid">
      <div class="service-card">
        ...
        <h3>Bebés & Crianças</h3>
        <p>Cuidados carinhosos para crianças dos 0 aos 11 anos, incluindo bebés recém-nascidos</p>
      </div>
      <div class="service-card">
        <div class="service-icon">🍳</div>
        <h3>Refeições Saudáveis</h3>
        <p>Com o meu curso de culinária, preparo refeições nutritivas e equilibradas para as crianças</p>
      </div>
      <div class="service-card">
        <div class="service-icon">🎨</div>
        <h3>Recreação</h3>
        <p>Atividades educativas e brincadeiras criativas adaptadas à idade de cada criança</p>
      </div>
      <div class="service-card">
        <div class="service-icon">🌙</div>
        <h3>Pernoite</h3>
        <p>Disponível para cuidados noturnos com toda a segurança e tranquilidade</p>
      </div>
      <div class="service-card">
        <div class="service-icon">📚</div>
        <h3>Apoio Escolar</h3>
      ...
      <div class="service-card">
        <div class="service-icon">🚶</div>
        <h3>Passeios</h3>
        <p>Acompanhamento em passeios e atividades ao ar livre por Lisboa</p>
      </div>
    </div>
  </div>
</section>
    ...
    <div class="booking-form">
      ...
      <form method="POST" action="/agendar">
        ...
        <div class="form-grid">
          ...
          <div>
            <label>Idade da criança * (0–11 anos)</label>
            <input type="number" name="child_age" placeholder="3" min="0" max="11" required>
          </div>
          <div>
            <label>Data *</label>
            <input type="date" id="booking_date" name="date" min="{today}" max="{max_date}" required>
          </div>
          <div>
          ...
          <div>
            ...
            <input type="number" id="hours_input" name="hours" placeholder="2" min="1" max="12" required>
            <div class="time-preview" id="time-preview"></div>
          </div>
          <div class="form-full">
...
<section id="chatbot" style="background:var(--white);padding:5rem 2rem">
  <div style="max-width:600px;margin:0 auto">
    <div class="section-header">
      ...
      <p class="section-sub">A minha assistente responde em segundos. Se não conseguir, podes chamar um agente humano.</p>
    </div>
...
def page_login(error=""):
    ...
    if GOOGLE_CLIENT_ID:
        ...
        <div class="divider">ou acede com email</div>"""
...
def page_admin(section="dashboard", msg="", msg_type="ok"):
    ...
    if section == "dashboard":
        rows = "".join(f"""<tr>
          ...
          <td>{b["date"]}</td>
          <td>{b.get("start_time","—")} → {b.get("end_time","—")}<br>
              <small style="color:var(--gray)">{b["hours"]}h</small></td>
        ...
        </tr>""" for b in bookings[:10]) or \
            '<tr><td colspan="8" style="color:var(--gray)">Sem agendamentos.</td></tr>'
        ...
        <h2 style="font-family:'Playfair Display',serif;margin-bottom:1rem">Agendamentos</h2>
        <table class="admin-table">
          <thead><tr>
            <th>#</th><th>Nome</th><th>Criança</th><th>Idade</th>
            <th>Data</th><th>Horário</th><th>Estado</th><th>Ações</th>
          </tr></thead>
          <tbody>{rows}</tbody>
        </table>"""
        ...
        </tr>""" for u in users) or \
            '<tr><td colspan="4" style="color:var(--gray)">Sem clientes.</td></tr>'
        content = f"""{msg_html}
        ...
        rows = "".join(f"""<tr style="{'background:#fffbeb' if not r['approved'] else ''}">
          <td>{r["id"]}</td>
          <td>{r["family"]}</td>
          ...
          <td style="display:flex;gap:.4rem;flex-wrap:wrap">
            {"".join([f'<form method="POST" action="{ADMIN_SECRET_PATH}/review-action" style="display:inline"><input type="hidden" name="id" value="{r["id"]}"><input type="hidden" name="action" value="approve"><button class="btn btn-sm btn-success">Aprovar</button></form>']) if not r["approved"] else ""}
            ...
            '<tr><td colspan="6" style="color:var(--gray)">Sem avaliações.</td></tr>'
        content = f"""{msg_html}
        ...
        conv_list = "".join(f"""
        <div class="conv-item {'conv-active' if False else ''} {'conv-resolved' if e['resolved'] else ''}"
             onclick="loadChat({e['id']})" id="conv-{e['id']}">
          <div style="display:flex;justify-content:space-between;align-items:center">
            <strong>{e.get("user_name") or "Anónimo"}</strong>
            <span class="status-badge {'status-resolved' if e['resolved'] else 'status-pending'}" style="font-size:.7rem">
              {'✓' if e['resolved'] else '●'}
            </span>
          </div>
          <div style="font-size:.8rem;color:var(--gray);margin-top:.2rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">
            {e['question'][:50]}
          </div>
          <div style="font-size:.72rem;color:#d1d5db;margin-top:.2rem">{e['created_at'][:16]}</div>
        </div>""" for e in escalations) or \
            '<p style="padding:1rem;color:var(--gray);font-size:.85rem">Sem conversas ainda.</p>'

        content = f"""{msg_html}
        <h2 style="font-family:'Playfair Display',serif;margin-bottom:1rem">Chat com Clientes</h2>
        <div style="display:grid;grid-template-columns:280px 1fr;gap:1.5rem;height:600px">

          <!-- Lista de conversas -->
          <div style="background:var(--white);border-radius:var(--radius);
                      overflow-y:auto;box-shadow:0 2px 12px rgba(0,0,0,.05)">
            <div style="padding:.85rem 1rem;border-bottom:1px solid #f1f5f9;
                        font-size:.8rem;font-weight:600;color:var(--gray);
                        text-transform:uppercase;letter-spacing:.05em">
              Conversas ({len(escalations)})
            </div>
            <div id="conv-list">{conv_list}</div>
          </div>
          ...
          <div style="background:var(--white);border-radius:var(--radius);
            ...
            <!-- Header do chat -->
            <div id="chat-panel-header" style="padding:1rem 1.5rem;
                  border-bottom:1px solid #f1f5f9;display:flex;
                  justify-content:space-between;align-items:center">
              <div>
                <div id="chat-client-name" style="font-weight:600">
                  Seleciona uma conversa
                </div>
                <div id="chat-client-email" style="font-size:.8rem;color:var(--gray)"></div>
              </div>
              <div id="chat-actions" style="display:none;gap:.5rem;display:none">
                <button class="btn btn-sm btn-success" onclick="resolveChat()">✓ Resolver</button>
              </div>
            </div>

            <!-- Mensagens -->
            <div id="admin-chat-messages" style="flex:1;overflow-y:auto;
                  padding:1.2rem;display:flex;flex-direction:column;gap:.75rem">
              <div style="text-align:center;color:var(--gray);font-size:.9rem;margin:auto">
                👈 Seleciona uma conversa para começar
              </div>
            </div>

            <!-- Input -->
            <div id="chat-input-area" style="padding:1rem 1.2rem;
                  border-top:1px solid #f1f5f9;display:none;gap:.75rem">
              <input id="admin-chat-input" type="text"
                     placeholder="Escreve a resposta ao cliente..."
                     style="flex:1;border-radius:99px;padding:.65rem 1.2rem">
              <button onclick="sendAdminMsg()"
                      style="background:linear-gradient(135deg,var(--pink),var(--pink-d));
                             color:#fff;border:none;border-radius:99px;
                             padding:.65rem 1.4rem;cursor:pointer;font-weight:600;
        ...
        <style>
          .conv-item {{
            padding:.85rem 1rem; cursor:pointer; border-bottom:1px solid #f9fafb;
            transition:background .15s;
          }}
          .conv-item:hover {{ background:#fdf4ff; }}
          .conv-item.conv-active {{ background:var(--pink-l); border-left:3px solid var(--pink); }}
          .conv-item.conv-resolved {{ opacity:.5; }}
          .admin-msg-bubble {{
            max-width:75%; padding:.65rem 1rem; border-radius:14px;
            font-size:.9rem; line-height:1.5;
          }}
          .admin-msg-bubble.from-client {{
            background:var(--pink-l); color:var(--dark);
            border-bottom-left-radius:3px; align-self:flex-start;
          }}
          .admin-msg-bubble.from-admin {{
            background:linear-gradient(135deg,var(--pink),var(--pink-d));
            color:#fff; border-bottom-right-radius:3px; align-self:flex-end;
          }}
          .admin-msg-bubble.from-system {{
            background:#f1f5f9; color:var(--gray); font-style:italic;
            font-size:.82rem; align-self:center; border-radius:8px;
          }}
          .admin-msg-time {{
            font-size:.7rem; color:#d1d5db; margin-top:.2rem;
          }}
        </style>
        ...
        <script>
          ...
          async function loadChat(eid) {{
            currentEscalationId = eid;

            // Highlight conversa activa
            document.querySelectorAll('.conv-item').forEach(el => el.classList.remove('conv-active'));
            document.getElementById('conv-' + eid)?.classList.add('conv-active');

            // Buscar info da escalação
            const res  = await fetch('/api/admin/chat/' + eid);
            const data = await res.json();

            document.getElementById('chat-client-name').textContent = data.user_name || 'Anónimo';
            document.getElementById('chat-client-email').textContent = data.user_email || '';
            document.getElementById('chat-actions').style.display = 'flex';
            document.getElementById('chat-input-area').style.display = data.resolved ? 'none' : 'flex';

            renderMessages(data.messages);

            // Polling a cada 2s
            if (pollInterval) clearInterval(pollInterval);
            pollInterval = setInterval(() => pollMessages(eid), 2000);
          }}

          async function pollMessages(eid) {{
            if (currentEscalationId !== eid) return;
            const res  = await fetch('/api/admin/chat/' + eid);
            const data = await res.json();
            renderMessages(data.messages);
          }}
          ...
          function renderMessages(messages) {{
            ...
            // Só re-renderiza se número de msgs mudou
            ...
            messages.forEach(m => {{
              ...
              wrap.style.cssText = 'display:flex;flex-direction:column;' +
                (m.sender === 'admin' ? 'align-items:flex-end' : 'align-items:flex-start');
              const bubble = document.createElement('div');
              bubble.className = 'admin-msg-bubble from-' + m.sender;
              bubble.textContent = m.message;
              const time = document.createElement('div');
              time.className = 'admin-msg-time';
              time.textContent = m.created_at.slice(0,16);
          ...
          async function sendAdminMsg() {{
            ...
            if (!msg || !currentEscalationId) return;
            input.value = '';
            await fetch('/api/admin/chat/' + currentEscalationId + '/send', {{
              method: 'POST',
              headers: {{ 'Content-Type': 'application/json' }},
              body: JSON.stringify({{ message: msg }})
            }});
            pollMessages(currentEscalationId);
          }}

          async function resolveChat() {{
            if (!currentEscalationId) return;
            if (!confirm('Marcar esta conversa como resolvida?')) return;
            await fetch('/api/admin/chat/' + currentEscalationId + '/resolve', {{ method: 'POST' }});
            location.reload();
          }}
        ...
        rows = "".join(f"""<tr>
          <td>{s["id"]}</td><td>{s["name"]}</td><td>{s.get("email") or "—"}</td>
        ...
        </tr>""" for s in staff_list) or \
            '<tr><td colspan="5" style="color:var(--gray)">Sem funcionários.</td></tr>'
        content = f"""{msg_html}
    ...
    def badge(s):
        n = pending_reviews if s == "reviews" else open_escalations if s == "help" else pending_bookings if s == "dashboard" else 0
        return f'<span style="background:var(--pink);color:#fff;border-radius:99px;padding:.1rem .5rem;font-size:.72rem;margin-left:auto">{n}</span>' if n else ""

    sidebar_nav = "".join(
        f'<a href="{ADMIN_SECRET_PATH}?s={s}" class="{"active" if section == s else ""}">{icon} {label}{badge(s)}</a>'
...
<div class="admin-layout">
  ...
  <main class="admin-main">
    <div class="admin-header">
      <h1>{ {"dashboard":"Dashboard","clients":"Clientes","reviews":"Avaliações","help":"Pedidos de Ajuda","staff":"Funcionários"}.get(section, section) }</h1>
    </div>
    {content}
  </main>
</div>
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/":
            ...
            self.send_html(page_login()) if not session else self.redirect("/")
        elif path == "/logout":
            destroy_session(self)
            self.redirect("/", headers={"Set-Cookie": "sid=; Max-Age=0; Path=/; HttpOnly"})
        elif path == "/auth/google":
            self.redirect(google_auth_url()) if GOOGLE_CLIENT_ID else self.redirect("/login")
        elif path == "/auth/google/callback":
            code   = params.get("code", [""])[0]
            tokens = google_exchange_code(code) if code else {}
            guser  = google_get_user(tokens.get("access_token", "")) if tokens else {}
            if not guser.get("email"):
                return self.send_html(page_login("Erro ao autenticar com Google."))
            uid   = get_or_create_user(guser)
            token = create_session(user_id=uid)
            self.redirect("/", headers={"Set-Cookie": f"sid={token}; Max-Age=28800; Path=/; HttpOnly"})
        elif path == ADMIN_SECRET_PATH:
            if self.is_admin_session():
                self.send_html(page_admin(params.get("s", ["dashboard"])[0]))
            else:
                self.send_html(self._admin_login_page())
        elif path == f"{ADMIN_SECRET_PATH}/logout":
            destroy_session(self)
            self.redirect("/", headers={"Set-Cookie": "sid=; Max-Age=0; Path=/; HttpOnly"})

        # ── API: Admin busca mensagens de uma conversa ──
        elif path.startswith("/api/admin/chat/") and not path.endswith("/send") and not path.endswith("/resolve"):
            ...
            with get_db() as db:
                esc = db.execute("SELECT * FROM chat_escalations WHERE id=?", (eid,)).fetchone()
                if not esc:
                    return self.send_json({"error": "not found"}, 404)
                msgs = [dict(m) for m in db.execute(
                    "SELECT * FROM chat_messages WHERE escalation_id=? ORDER BY id ASC", (eid,)
                ).fetchall()]
            self.send_json({
                "id": esc["id"],
                ...
                "resolved": bool(esc["resolved"]),
                "messages": msgs
            })

        # ── API: Cliente busca mensagens da sua conversa (polling) ──
            ...
            with get_db() as db:
                msgs = [dict(m) for m in db.execute(
                    ...
                    "WHERE escalation_id=? AND sender='admin' ORDER BY id ASC", (eid,)
                ).fetchall()]
                esc = db.execute("SELECT resolved FROM chat_escalations WHERE id=?", (eid,)).fetchone()
            self.send_json({
                "messages": msgs,
                "resolved": bool(esc["resolved"]) if esc else False
            })

        else:
            self.send_html("<h2 style='text-align:center;margin-top:4rem;font-family:serif'>404 – Não encontrado</h2>", 404)
    ...
    def do_POST(self):
        ...
        if path == "/api/chat":
            ...
            if reply.startswith("ESCALAR:"):
                ...
                with get_db() as db:
                    ...
                    eid = cur.lastrowid
                    # Guardar histórico da conversa como mensagens do cliente
                    for m in history[-6:]:
                        if m.get("role") == "user":
                            db.execute(
                                "INSERT INTO chat_messages (escalation_id, sender, message) VALUES (?,?,?)",
                                (eid, "client", m["content"][:500])
                            )
                    db.commit()
                return self.send_json({"reply": "", "escalated": True, "eid": eid})
            return self.send_json({"reply": reply, "escalated": False})

        # ── Escalação manual do botão "Chamar agente" ──
        elif path == "/api/chat/escalate":
            body    = read_json(self)
            history = body.get("history", [])
            last_q  = next((m["content"] for m in reversed(history) if m["role"] == "user"), "Pedido manual")
            with get_db() as db:
                cur = db.execute(
                    "INSERT INTO chat_escalations (user_name, user_email, question) VALUES (?,?,?)",
                    (session.get("name") if session else None,
                     session.get("email") if session else None,
                     f"[Pedido manual] {last_q}")
                )
                eid = cur.lastrowid
                for m in history[-6:]:
                    if m.get("role") == "user":
                ...
                db.commit()
            return self.send_json({"ok": True, "eid": eid})

        # ── Cliente envia mensagem numa conversa activa ──
            ...
            try:
                ...
                return self.send_json({"error": "invalid"}, 400)
            body = read_json(self)
            msg  = body.get("message", "")[:500].strip()
            if not msg:
                return self.send_json({"error": "empty"}, 400)
            with get_db() as db:
                esc = db.execute("SELECT resolved FROM chat_escalations WHERE id=?", (eid,)).fetchone()
                if not esc or esc["resolved"]:
                    return self.send_json({"error": "closed"}, 400)
                db.execute(
                    "INSERT INTO chat_messages (escalation_id, sender, message) VALUES (?,?,?)",
                    (eid, "client", msg)
                )
        ...
        elif path.startswith("/api/admin/chat/") and path.endswith("/send"):
            if not self.is_admin_session():
                return self.send_json({"error": "unauthorized"}, 403)
            try:
        ...
        # ── Admin resolve conversa ──
        elif path.startswith("/api/admin/chat/") and path.endswith("/resolve"):
            if not self.is_admin_session():
                return self.send_json({"error": "unauthorized"}, 403)
            try:
                eid = int(path.split("/")[-2])
        ...
        # ── Agendamento ──
            ...
            bdate       = field(data, "date")
            start_time  = field(data, "start_time")
            hours       = field(data, "hours")
            notes       = field(data, "notes")
            user_id     = field(data, "user_id") or None

            if not all([parent_name, address, child_name, child_age, bdate, start_time, hours]):
                return self.send_html(page_home(session, booking_msg="Preenche todos os campos obrigatórios.", booking_type="err"))
            try:
                age_int   = int(child_age); assert 0 <= age_int <= 11
                hours_int = int(hours);     assert 1 <= hours_int <= 12
                # Validar data não é no passado
                booking_date = date.fromisoformat(bdate)
                assert booking_date >= date.today()
            except Exception:
                return self.send_html(page_home(session, booking_msg="Dados inválidos. Verifica a data e os valores.", booking_type="err"))

            end_time = calc_end_time(start_time, hours_int)
            with get_db() as db:
                db.execute(
                    "INSERT INTO bookings (user_id,parent_name,address,child_name,child_age,date,start_time,end_time,hours,notes) "
                    "VALUES (?,?,?,?,?,?,?,?,?,?)",
                    (user_id, parent_name, address, child_name, age_int, bdate, start_time, end_time, hours_int, notes)
                )
                db.commit()
            self.send_html(page_home(session,
                booking_msg=f"✅ Pedido enviado! Agendado para {bdate} das {start_time} às {end_time}.", booking_type="ok"))
        ...
        # ── Avaliação ──
            ...
            with get_db() as db:
                db.execute("INSERT INTO reviews (family,stars,comment) VALUES (?,?,?)", (family, stars_int, comment))
                db.commit()
            self.send_html(page_home(session, review_msg="💛 Obrigada! Ficará visível após aprovação.", review_type="ok"))

        # ── Admin login ──
        elif path == f"{ADMIN_SECRET_PATH}/login":
            data = read_body(self)
            pwd  = field(data, "password")
            if hashlib.sha256(pwd.encode()).hexdigest() == hashlib.sha256(ADMIN_PASSWORD.encode()).hexdigest():
                token = create_session(is_admin=True)
                self.redirect(ADMIN_SECRET_PATH, headers={"Set-Cookie": f"sid={token}; Max-Age=14400; Path=/; HttpOnly"})
            else:
                self.send_html(self._admin_login_page("Senha incorreta."))

        elif path == f"{ADMIN_SECRET_PATH}/booking-action":
            if not self.is_admin_session(): return self.redirect(ADMIN_SECRET_PATH)
            data   = read_body(self)
            status = "accepted" if field(data, "action") == "accept" else "rejected"
            with get_db() as db:
                db.execute("UPDATE bookings SET status=? WHERE id=?", (status, field(data, "id")))
                db.commit()
            ...
            data = read_body(self)
            with get_db() as db:
                db.execute("UPDATE chat_escalations SET assigned_to=? WHERE id=?",
                           (field(data, "staff_id") or None, field(data, "id")))
                db.commit()
            ...
            data = read_body(self)
            with get_db() as db:
                db.execute("UPDATE chat_escalations SET resolved=1 WHERE id=?", (field(data, "id"),))
                db.commit()
            self.redirect(f"{ADMIN_SECRET_PATH}?s=help")
            ...
            if name:
                with get_db() as db:
                    db.execute("INSERT INTO staff (name, email) VALUES (?,?)", (name, field(data, "email")))
                    db.commit()
            ...
            data = read_body(self)
            with get_db() as db:
                db.execute("DELETE FROM staff WHERE id=?", (field(data, "id"),))
                db.commit()
            self.redirect(f"{ADMIN_SECRET_PATH}?s=staff")
...
<head>
  ...
  <title>Admin</title><style>{STYLE}</style>
</head>
<body>
<div class="login-page">
  <div class="login-card">
...
if __name__ == "__main__":
    ...
    print("    ⚠️  Define ANTHROPIC_API_KEY e GOOGLE_CLIENT_ID no Railway!")
