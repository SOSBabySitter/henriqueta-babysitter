...
async function requestAgent() {
  try {
    ...
    appendMsg('✅ Pedido enviado! Um funcionário entrará em contacto em breve.', 'bot');
  } catch {
...
def page_admin(section="dashboard", msg="", msg_type="ok"):
    ...
    if section == "dashboard":
        ...
        rows = "".join(f"""<tr>
          ...
          <td><span class="status-badge {"status-resolved" if e["resolved"] else "status-pending"}">
            {"Resolvido" if e["resolved"] else "Aberto"}</span></td>
          <td>
        ...
        </tr>""" for e in escalations) or \
            '<tr><td colspan="6" style="color:var(--gray)">Sem pedidos de ajuda.</td></tr>'
        content = f"""{msg_html}
        ...
        if path == "/api/chat":
            ...
            if reply.startswith("ESCALAR:"):
                ...
                with get_db() as db:
                    ...
                    db.commit()
                return self.send_json({"reply": "", "escalated": True})
